...
/**
 * The ONLY module in the app that imports onnxruntime-web.
 *
...
// Threads need SharedArrayBuffer, which needs cross-origin isolation. Safari
// has no `credentialless` COEP, so it lands here single-threaded. Threads are
// an optimization, never a requirement.
ort.env.wasm.numThreads = globalThis.crossOriginIsolated
  ? Math.min(4, navigator.hardwareConcurrency || 4)
  : 1;

let sessionPromise: Promise<{ session: ort.InferenceSession; ep: ExecutionProvider }> | null = null;
let modelUrl = '';
